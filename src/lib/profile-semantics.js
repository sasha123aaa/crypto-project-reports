...
const PROFILE_COPY = Object.freeze({
  [PROJECT_CATEGORIES.INFRA]: {
    ...
    executive: (name) => [`${name} — ставка на капитал и использование сети.`, "TVL, пользователи и комиссии должны подтверждать инфраструктурную роль.", "Ключевой вопрос — оправдывает ли сетевая экономика оценку актива?"],
  ...
  [PROJECT_CATEGORIES.MEME]: {
    ...
    executive: (name) => [`${name} — ставка на устойчивость рыночного внимания.`, "Оборот и ликвидность определяют качество входа и выхода.", "Концентрация и потеря импульса создают риск резкого unwind."],
  ...
  [PROJECT_CATEGORIES.UTILITY]: {
    ...
    executive: (name) => [`${name} ценен, только если продукт создает спрос на токен.`, "Utility и adoption должны быть измеримыми, а не декларативными.", "Ликвидность и оценка определяют качество инвестиционного входа."],
...
const PROJECT_COPY = Object.freeze({
  eth: {
    executive:["ETH — зрелый базовый актив с глубокой ликвидностью и капиталом.", "Комиссии и пользователи должны подтверждать спрос на Ethereum.", "Ключевой вопрос — сколько ценности ETH удерживает между L1, L2 и приложениями."],
  ...
  sol: {
    executive:["SOL — ставка на быстрый рост on-chain экономики Solana.", "Капитал, оборот и комиссии должны подтверждать качество роста.", "Ключевой вопрос — сколько создаваемой экосистемой ценности остается у SOL."],
  ...
  doge: {
    executive:["DOGE — зрелый meme-актив с узнаваемым и ликвидным рынком.", "Оборот и глубина выхода важнее фундаментального денежного потока.", "Ключевой риск — концентрация и потеря внимания к активу."],
  ...
  pepe: {
    executive:["PEPE — meme-актив, напрямую зависимый от рыночного импульса.", "Высокий оборот поддерживает позицию, пока сохраняется внимание.", "Ключевой риск — одновременная потеря импульса и ликвидности."],
