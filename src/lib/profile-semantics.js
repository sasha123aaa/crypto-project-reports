...
const PROFILE_COPY = Object.freeze({
  [PROJECT_CATEGORIES.INFRA]: {
    ...
    verdict: (name, ticker) => ({ title:`Инвестиционный вывод по ${ticker}`, subtitle:"Тезис: сеть должна удерживать капитал и ценность", paragraphs:[`${name} остается убедительным, пока рост капитала и использования переходит в спрос на базовый актив; главный риск — разрыв между экономикой сети и оценкой токена.`] }),
  ...
  [PROJECT_CATEGORIES.DEFI]: {
    ...
    verdict: (name, ticker) => ({ title:`Инвестиционный вывод по ${ticker}`, subtitle:"Тезис: продуктовая экономика должна работать на токен", paragraphs:[`${name} интересен при устойчивом использовании и понятном захвате ценности токеном; главный риск — рост продукта без измеримой выгоды для держателя.`] }),
  ...
  [PROJECT_CATEGORIES.MEME]: {
    ...
    verdict: (name, ticker) => ({ title:`Инвестиционный вывод по ${ticker}`, subtitle:"Тезис: ликвидность должна переживать смену внимания", paragraphs:[`${name} остается торговым тезисом, пока оборот обеспечивает реальный выход из позиции; главный риск — одновременная потеря внимания и ликвидности.`] }),
  ...
  [PROJECT_CATEGORIES.UTILITY]: {
    ...
    verdict: (name, ticker) => ({ title:`Инвестиционный вывод по ${ticker}`, subtitle:"Тезис: использование продукта должно создавать спрос на токен", paragraphs:[`${name} убедителен при измеримой связи продукта со спросом на токен; главный риск — рост использования без выгоды для держателя.`] }),
  ...
  [PROJECT_CATEGORIES.CONSUMER]: {
    ...
    verdict: (name, ticker) => ({ title:`Инвестиционный вывод по ${ticker}`, subtitle:"Тезис: удержание должно переходить в экономику токена", paragraphs:[`${name} убедителен при устойчивом удержании и понятной связи продукта с токеном; главный риск — аудитория без монетизации и спроса на актив.`] }),
...
const PROJECT_COPY = Object.freeze({
  eth: {
    ...
    verdict:{ title:"Инвестиционный вывод по ETH", subtitle:"Тезис: расчетный капитал должен создавать спрос на ETH", paragraphs:["ETH сохраняет сильную инфраструктурную позицию, пока расчетный капитал и безопасность поддерживают спрос на актив; главный риск — уход создаваемой ценности в L2 и приложения."]},
  ...
  sol: {
    ...
    verdict:{ title:"Инвестиционный вывод по SOL", subtitle:"Тезис: быстрый рост должен стать устойчивым спросом на SOL", paragraphs:["SOL выглядит убедительно, пока экосистема удерживает капитал и создает повторяемые комиссии; главный риск — быстрое охлаждение активности после смены цикла."]},
  ...
  doge: {
    ...
    verdict:{ title:"Инвестиционный вывод по DOGE", subtitle:"Тезис: зрелая ликвидность должна переживать смену цикла", paragraphs:["DOGE поддерживает позицию узнаваемостью и глубиной рынка; главный риск — затяжное снижение внимания, при котором ликвидность перестает компенсировать отсутствие денежного потока."]},
  },
  pepe: {
    ...
    verdict:{ title:"Инвестиционный вывод по PEPE", subtitle:"Тезис: импульс должен сохранять ликвидный выход", paragraphs:["PEPE остается сильным торговым тезисом, пока внимание поддерживает оборот и глубину рынка; главный риск — резкое исчезновение ликвидного выхода после потери импульса."]},
  },
});
