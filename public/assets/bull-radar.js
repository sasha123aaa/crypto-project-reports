(function () {
  ...
  let rawRows = [], rows = [], selectedId = null, chart = null, lastUpdated = null;
  const $ = (id) => document.getElementById(id);
  const money = (v) => Number.isFinite(Number(v)) ? new Intl.NumberFormat("en-US", { style:"currency", currency:"USD", maximumFractionDigits:Number(v) < 1 ? 6 : 2 }).format(Number(v)) : "—";
  const pct = (v) => Number.isFinite(Number(v)) ? `${Number(v) >= 0 ? "+" : ""}${Number(v).toFixed(2)}%` : "—";
  const compact = (v) => Number.isFinite(Number(v)) ? new Intl.NumberFormat("ru-RU", { notation:"compact", maximumFractionDigits:1 }).format(Number(v)) : "—";
