(function () {
  ...
  function strategyStatsHtml(row) { const st=rowStrategyStats(row); if(!st) return "Истории нет"; if(Number(st.totalTrades||0)<=0&&st.activeTrade) return `Активная сделка · ${escapeHtml(st.activeTrade.status||"активна")}`; if(Number(st.totalTrades||0)<=0) return "Истории нет"; return [`Сделок: ${escapeHtml(st.totalTrades)}`,`Тейков: ${escapeHtml(st.takeHits)}`,`Winrate: ${Number(st.winrate||0).toFixed(0)}%`,`Макс. уср: ${escapeHtml(st.maxActivatedLevels)}`,`Ср. просадка: ${pct(st.avgDrawdownPct)}`,`Закрытый результат: ${pct(st.estimatedFullCapitalResultPct)}`,st.activeUnrealizedFullCapitalResultPct!=null?`Активный результат: ${pct(st.activeUnrealizedFullCapitalResultPct)}`:null,`Лучший режим: ${escapeHtml(st.bestEntryMode)}`,`Активная сделка: ${st.activeTrade?escapeHtml(st.activeTrade.status||"да"):"нет"}`].filter(Boolean).join(" · "); }
